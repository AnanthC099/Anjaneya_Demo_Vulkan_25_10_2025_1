...
static VkResult CreateSceneDescriptorStuff(void)
{
    ...
    psize.descriptorCount = 2; // scene0 + scene1
    ...
    // alloc sets
    VkDescriptorSetLayout layouts[2] = { g.sUboDSL, g.sUboDSL };
...
static VkResult RecordFrameCmd(uint32_t imgIndex)
{
    ...
    // push constant float t
    vkCmdPushConstants(cmd, g.comp_pLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(float), &g.t);
